...
,m_pImageBuffer(NULL)
{
	if ((objCGXDevicePointer.IsNull())||(NULL == m_hWnd))
	{
	...
	m_hPenRegion = CreatePen(PS_SOLID, 2, RGB(0, 255, 0));
	m_hMemDC = NULL;
	m_hBitmap = NULL;
	m_nLastWidth = 0;
	m_nLastHeight = 0;
...
void CGXBitmap::__MonoPrepareForShowImg()
{
	...
	m_pBmpInfo->bmiHeader.biSize			= sizeof(BITMAPINFOHEADER);
	m_pBmpInfo->bmiHeader.biWidth			= (LONG)m_nImageWidth;
	m_pBmpInfo->bmiHeader.biHeight			= 0 - (LONG)m_nImageHeight;	
...
void CGXBitmap::__DrawImg2(BYTE* pBuffer, POINT &pt1, POINT &pt2)
{
	...
	// 如果窗口尺寸发生了变化，重新创建hMemDC
	if (nWndWidth != m_nLastWidth || nWndHeight != m_nLastHeight) {
		if (m_hMemDC != NULL) {
			::DeleteDC(m_hMemDC);
			m_hMemDC = NULL;
		}
		...
		m_nLastWidth = nWndWidth;
		m_nLastHeight = nWndHeight;

		m_hMemDC = ::CreateCompatibleDC(m_hDC);
		m_hBitmap = CreateCompatibleBitmap(m_hDC, nWndWidth, nWndHeight);
	...
	HPEN hOldPen = (HPEN)::SelectObject(m_hMemDC, m_hPenRegion);
	::MoveToEx(m_hMemDC, pt1.x, pt1.y, NULL);
	::LineTo(m_hMemDC, pt1.x, pt2.y);
	::LineTo(m_hMemDC, pt2.x, pt2.y);
	::LineTo(m_hMemDC, pt2.x, pt1.y);
	::LineTo(m_hMemDC, pt1.x, pt1.y);
...
*/
//----------------------------------------------------------------------------------
void CGXBitmap::Show(CImageDataPointer& objCImageDataPointer, POINT &pt1, POINT &pt2)
{
	GX_VALID_BIT_LIST emValidBits = GX_BIT_0_7;
	BYTE* pBuffer = NULL;
